...
3. **本机双窗口用 `BroadcastChannel`**：消息只在同浏览器同 origin 内传播，按 deckId + 随机 channel 隔离；发送前深拷 snapshot。
4. **画笔用轻量 SVG pointer-event 实现**：只做笔、高亮、颜色、粗细、整条笔迹橡皮擦、撤销和清空；坐标归一化到固定 16:9 viewBox，跨不同窗口尺寸仍能同步。不引入完整白板 SDK。
...
12. **远程直播是单向临时会话**：agent 内存 registry 按 userId+deckId 保留一个 8h 会话；演讲者用 token-bound owner API 节流发布完整 page/blackout/drawings snapshot，公开观众只读 SSE。旧演讲者窗口、旧 token、结束或删除 deck 后都不能继续写/读。
...
- 直播 token 同样使用 256-bit 随机值，不记录 token；日志只记 userId/deckId 与状态转移。
...
- 静态分享明确允许访客自行翻页；弹窗命名为「静态分享」，与直播观看区分。

### Task 16-C2：远程直播分享

- 演讲者视图创建、复制、结束 `/live/:token` 观看链接。
- owner update/end 同时校验 session user、deckId 和当前 token，防旧窗口污染新直播。
- 观众先加载最新 snapshot，再用 SSE 收 page/blackout/drawings revision；重连时补发完整状态。
- 观众端只有本机全屏控制，无翻页、overview、画笔、主题或演讲者入口。
- 会话 8h 过期；结束、替换、删除 deck 或 agent 重启后失效。SSE 走现有 nginx no-buffer 配置。
...
- `GET/POST /api/decks/:id/live`：owner 查询或创建当前直播。
- `PUT /api/decks/:id/live/:token/state`：owner 发布完整 snapshot；token 必须仍为当前会话。
- `DELETE /api/decks/:id/live/:token`：owner 结束当前直播。
- `GET /api/live/:token/presentation|events|assets/:assetId`：公开只读 payload、SSE 与 deck-scoped 资源。
...
1. `BroadcastChannel` 名称含 deckId + 128-bit 随机 channel，只用于本机双窗口。
...
6. 远程直播 registry 只保存临时 presentation snapshot；owner 写入同时匹配 userId + deckId + 256-bit token，公开端点没有 mutation 路径。
7. 直播 asset 查询绑定 token 对应 deckId；结束、替换、过期、删除 deck 后先失效 token，再拒绝读取 BLOB。
...
- [x] 远程直播观众无 Cookie、无控制入口；翻页、黑白屏、画笔、橡皮擦和结束事件真浏览器同步。
...
- agent：1094 tests 全量通过 + 5 个直播 registry/mount focused tests 通过；coverage lines 90.95% / branches 80.04% / functions 89.26%。
- creator：391 tests passed；coverage lines 83.93% / branches 72.40% / functions 75.53%。
- shared / slidev design system：3 / 55 tests passed。
- Playwright：63 passed、10 skipped；覆盖原生放映、静态/直播分享、双用户并发、桌面/移动端与 12 组视觉基线。
