...
- ❌ 外部格式导入（Markdown / PPTX）— 2026-07-11 产品决策：永久不做
- ❌ Deck 分享链接、权限、协同编辑 — 延 Phase 11 / Phase 17+
...
- ❌ 新增第三套模板
- ❌ 用户自定义模板（永久不做或留 Phase 17+，长期愿景见 [vision.md](./vision.md#远期愿景模板生态系统)）
- ❌ 模板市场 / 分享
...
**核心约束**：本 Phase 只做架构与重构，**不做**完整生态化（脚手架 / 市场 / npm 公开包），那些依赖足够多模板与第三方意愿，留 Phase 17+。
...
- ❌ 模板创作脚手架 CLI / 模板市场 / 创作者经济（全部留 Phase 17+）
- ❌ 公共组件提 npm 公开包发布 / 模板 override 机制（同上 Phase 17+）
- ❌ 新增第三套模板（验证两套切换无损就够）
...
- 9-B A01 路由鉴权：盘点出 8 个公开端点（plan 原本只列 2 个）含 `POST /api/call-tool` 严重漏洞（未登录可调 write_slides / mcp\_\_\* 等敏感工具）+ slides.md 跨用户读漏洞，全部加 requireAuth + slides 端点加持锁守卫；agent +13 测
...
## Phase 11.8：锚图选样 — 跨 slide AI 生图风格统一

> 详细实施:[plan 32-phase11.8-anchor-image-styling.md](../plans/32-phase11.8-anchor-image-styling.md)
> 前置:Phase 11.5/11.6/11.7 ✅;hybrid vision-aware 通道 2026-05-18 真打 smoke 已 verify
> 后续:无强依赖

**目标**:解决 dogfood 反复反馈的「同一份 deck 22 张 AI 生图风格不统一」问题。Phase 11.6 加的 `buildStructuredImagePrompt` 只通过 prompt 注入 palette / styleHint 约束,**palette 维度对齐但构图 / 笔触粒度 / 元素密度仍跨页漂移**。本 Phase 加锚图选样:首次走 image-gen 路径前,**主 LLM 看 deck 大纲现场生成 3 个差异化样张 prompt** → 并发出 3 图让用户挑/换批,选定锚图持久化到 deck,后续所有 `generate_slide_image` 自动注入锚图走 hybrid vision-aware(vision encoder 真"看到"风格而非仅读 prompt 描述),跨 slide 视觉对齐。**配套**加 hybrid 失败三级降级(hybrid → 路 B + palette 约束 → fallback-rewrote),避免单页 vision 抖动就丢图。

**关键决策**:

- **样张主题:主 LLM 看 deck 大纲现场出 3 个差异化 prompt,工具层不预设风格字符串**(2026-05-19 用户反馈推翻原硬编 3 套预设方案 — 工程师预设 = 产品永久天花板 + 跟用户实际业务脱节);LLM 输出 `{ style, prompt } × 3` JSON,sanity check pairwise style label 相似度 ≥ 0.8 视为雷同 retry 一次
- **"换一批" per-deck 限 3 次**:每次烧 ~$0.12,上限 ~$0.48;3 次还不满意基本是 prompt 工程问题非抽样问题
- **hybrid 失败三级降级**:路 A + image_input(hybrid 带 anchor)→ 路 B + 纯 text + palette 约束(buildStructuredImagePrompt 保留)→ fallback-rewrote 兜底。anchor 风格丢失但 palette 维度仍对齐,比直接换组件版好得多
- **锚图持久化 `decks.anchor_asset_id` 单字段**:一个 deck 一个 anchor;asset 走既有 deck_assets BLOB 通路;FK `onDelete: 'set null'`
- **切模板时清空 anchor,undo 切模板时恢复**:`deck_versions` 加 `anchor_asset_id` 列跟 `template_id` 同套路 snapshot
- **协议层 LLM 不感知 anchor**:`generate_slide_image` 工具同步入口自动读 deck.anchor_asset_id 注入 baseImage;**优先级 frontmatter imageSrc > anchor > none**;系统/工具 prompt 完全不改
- **UI 阻塞流程**:write_slides 完成后(image LLM 已配 + anchor === null)useAIChat 暂停 → 弹 AnchorPickerModal → 用户选定/跳过 → LLM session 恢复继续派发 N 张 generate_slide_image
- **顶栏"重选风格"按钮**:image LLM 已配 + deck 有 anchor 时显示,可主动触发重选

**包含内容**:

- DB schema:`decks.anchor_asset_id` + `deck_assets.purpose` + `deck_versions.anchor_asset_id` 三列
- 后端:`mood-board/index.ts` 生成器(**调主 LLM 出 3 个差异化 prompt + sanity check retry** + 并发 generateImage)+ `mood-board/prompt.ts` 系统 prompt + JSON schema + outline 截取 + style 相似度算法 + `routes/mood-board.ts` 两端点(generate / select anchor 限频 + IDOR guard)+ `generate-slide-image` 工具入口加 anchor 读取分支 + `openai-image.ts` 三级降级 + `image-gen-job` 加 path-B-fallback 日志事件 + `template-switch-job` / `routes/decks` restore 端点接 anchor 同步
- 前端:`AnchorPickerModal.vue` + `useMoodBoardPicker.ts` composable + `useAIChat` 加 LLM session 暂停/恢复信号 + 顶栏"重选风格"按钮 + `shared/api.ts` 加新类型 + `api/decks.ts` 加调用方法
- E2E:`anchor-image-flow.spec.ts`(stub 模式 4 case:正常 / 跳过 / 限频 / 工具透传)

**不在范围**:

- ❌ **多 anchor / 章节级 anchor**:一 deck 一 anchor 是 mvp 语义,多 anchor 留 Phase 17+
- ❌ **用户上传自己的图作为 anchor**:复用 user_assets 通路看似简单,UX / 校验 / 失败兜底都要单独想,留 Phase 17+
- ❌ **LLM 自动选 anchor**(基于 deck 主题决定风格):跟"用户挑调性"产品定位冲突
- ❌ **跨 deck 复用 anchor**:同用户多 deck 主题各异,统一 anchor 反而限制创造
- ❌ **mood-board-discarded asset GC cron**:本 Phase 暂留 DB,Phase 17+ 跟其它 GC 一并清
- ❌ **anchor 编辑**(用户改几像素再用):scope 失控

**预计工作量**:8 task(A: DB / B: mood-board 生成器 / C: 路由 / D: 工具层 anchor 注入 / E: 三级降级 / F: 前端 modal + composable / G: useAIChat 接驳 / H: E2E + dogfood),~1 周
...
- ❌ MCP server 健康监控 / SLA 看板（留 Phase 17+）
...
> **2026-05-18 重定义**：Phase 15 改为「归档数据包 export + import 双向闭环」。原 Markdown / PPTX 外部格式导入方向已于 2026-07-11 取消，永久不做；Phase 15 仅保留 Lumideck 原生 `.lumideck` 归档闭环。
...
- **静态分享 ✅ 落地**：访客可自行翻页，链接具备过期、撤销和公开资源隔离
- **直播分享 ✅ 落地**：远程观众只读跟随演讲者的翻页、黑白屏与标注，不能反向操作
...
- **演讲者壳子组件 `<PresenterMode>`**：复刻日常演讲核心 UI
  ...
  - 黑屏 / 白屏切换
  - 画笔 / 高亮工具（drawing layer，覆盖在 slide 上的 SVG / canvas）
  - 缩略图 overview 网格（Slidev `Esc` 键打开的那个）
- **按场景隔离观看权限**：owner 放映要求登录；静态链接允许访客自行浏览；直播链接只允许被动跟随，三者共用同一渲染口径
  - `:id` 路径：要求 session cookie + deck owner 校验（编辑者本人 fullscreen 用）
  - `:slug` 路径：查 share_links 表，未过期 + 未撤销则 200 公开（公开分享用，无需登录）
  - 直播路径：随机链接只读加载当前演示状态，后续单向跟随演讲者
- **编译产物部署**：本组件随 creator 一起 `vite build`，产物是普通 SPA chunk，nginx 静态托管即可
- **slides.md 全局文件 + slidev-lock 彻底退役**：每个用户 / 分享访问独立 Vue 实例，互不干扰；多用户同时全屏放映各自的 deck **不再互斥**

**交付物**：

- `packages/creator/src/presentation/`：
  - `PresentationViewer.vue`：主组件，接 deck markdown + templateId + 模式（present / share-view），渲染单页 + 翻页 + 键盘交互
  - `PresenterMode.vue`：演讲者视图（split + 备注 + 计时器）
  - `DrawingLayer.vue`：画笔 / 高亮覆盖层
  - `OverviewGrid.vue`：缩略图网格（Esc 切换）
- 直播观看：演讲者管理直播链接，远程观众页面只展示同步状态与本机全屏
...
- [x] 直播链接无需登录；远程观众无翻页/标注入口，并同步演讲者翻页、黑白屏、画笔与橡皮擦
...
- **远程直播**：演讲者单向发布状态；观众凭随机链接只读订阅，结束直播或删除 deck 后链接立即失效
- **PDF / PPTX 导出**：Phase 14 已先完成完全 client-side 导出，不依赖 Slidev CLI

**不做什么**：

- ❌ Slidev 所有边角功能（录屏 / monaco code editor 在 slide 内 / canvas 物理模拟等）—— 只复刻日常用得到的核心 80%
- ❌ 多人实时协作 drawing（Phase 17+ 远期）
- ❌ 自定义 transition 动画（先用 Vue Transition 默认 fade，需要再扩）
...
> **状态**：✅ 已完成（2026-07-11，[plan 34](../plans/34-phase17-image-style-library.md)）

**目标**：结束“每个 deck 必须等待 3–5 分钟抽三张风格”的主流程。系统预设和用户保存风格即时可选，AI 探索只在用户主动触发时发生，并形成跨 deck 复用闭环。

**交付物**：

- 至少 12 套经过验证的系统配图风格，打开即展示、点击即应用
- “系统预设 / 我的风格 / AI 探索”三段式风格库
- 探索结果保存、重命名、删除与跨 deck 复用
- 浏览和探索不锁聊天；首次尚未决策才阻塞生图工具
- 系统/用户风格应用时复制为 deck-local anchor，删除源资产不影响已应用 deck
- `.lumideck` schema v2 完整保留当前锚图与风格状态，并继续兼容 v1

**验收条件**：

- [x] 打开风格库不调用 LLM 或图片模型，系统预设 1 秒内可选
- [x] 新 deck 可直接复用历史保存风格，不再重新抽卡
- [x] AI 探索仅显式触发，等待期间可关闭风格库并继续编辑
- [x] 跨用户、跨 deck 资产隔离测试全绿
- [x] 归档 v2 round-trip 保留风格，v1 仍可导入

**不做什么**：

- ❌ 上传任意参考图创建风格、团队共享风格库、风格市场
- ❌ 持久化探索 job/SSE、逐张渐进展示
- ❌ 对历史图片一键批量重绘、多 anchor 或章节级风格
...
> **2026-07-11**：永久移除 Markdown / PPTX 外部格式导入方向，不再放入任何后续 Phase；跨账号、跨实例备份恢复只保留已上线的 `.lumideck` 原生归档闭环。原因：外部格式转换质量不可控，也不是当前真实使用流程的核心痛点。
>
> **2026-07-11**：启动 Phase 17 配图风格资产库与选择流程重构。系统预设与已保存风格成为即时主路径，原每 deck 自动生成三张样图降为用户主动探索；原“Phase 17+ 远期可能”顺延为 Phase 18+。
>
> **2026-07-11**：完成 Phase 17。落地 12 套系统风格、用户风格跨 deck 资产、显式后台 AI 探索、`.lumideck` v2、三段式选择 UI 与完整回归/E2E；旧的自动抽卡必经流程正式退役。
...
| 2026-05-18 | **新增 Phase 11.8(锚图选样)**:dogfood 反复反馈"22 页 AI 生图风格不统一",`buildStructuredImagePrompt` 仅 prompt 注入 palette 约束,palette 维度对齐但构图 / 笔触 / 元素密度跨页漂移。同日真打 smoke([commit `d8b2ae5`](../../packages/agent/src/llm/__tests__/smoke/hybrid-image-edit-smoke.test.ts))验证 hybrid vision-aware 通道(`input_image` 走 vision encoder 非 token 化)端到端可用。新插 Phase 11.8:首次走 image-gen 前预生成 3 张样张 → 用户挑 → 选定 anchor 持久化到 `decks.anchor_asset_id` → 后续 `generate_slide_image` 自动注入锚图走 hybrid vision-aware 跨 slide 对齐。**核心抉择**:(1) ~~抽象 mood board 3 套预设~~(2026-05-19 推翻,见下条);(2) 换批硬限 3 次,上限 ~$0.48;(3) hybrid 失败三级降级(路 A → 路 B + palette 约束 → fallback-rewrote)避免单页 vision 抖动丢图;(4) 切模板清空 anchor + undo 恢复(`deck_versions.anchor_asset_id` 跟 `template_id` 同源套路);(5) LLM 不感知 anchor — 工具同步入口自动读注入 baseImage,优先级 `frontmatter imageSrc > anchor > none`。Plan 详见 [plan 32](../plans/32-phase11.8-anchor-image-styling.md),8 task,~1 周。插入位置:Phase 11.7 之后 / 废弃的 Phase 11 之前,与 11.5/11.6/11.7 image-gen 系列保持语义连续                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               | 用户对前一轮"hybrid 是不是真走 vision 不被 token 化"质疑触发真打 smoke 验证,绿后用户主动提出"预选 3 张风格底材后续按其生成"思路;Claude 评估可行(技术栈现成 + hybrid 已 verify),澄清三个设计抉择(mood board 主题 / 三级降级 / UI 流程)后用户拍板写 plan                                                                                           |
| 2026-05-19 | **Phase 11.8 抉择 1 推翻:样张主题改主 LLM 看大纲现场出 3 个差异化 prompt**。用户指出原"工程师硬编 3 套预设(flat-geometric / soft-watercolor / minimalist-line-art)"两条致命问题:(1) **产品永久天花板** — 用户实际想要的 Bauhaus / 浮世绘 / isometric / 手绘 / art deco 等无穷风格永远进不来;(2) **跟用户实际业务脱节** — deck 大纲已经在 DB 里(write_slides 完成后),却拿无业务元素的抽象 mood board 让用户挑,挑的"调性"在后续业务图里能 transfer 多少不可知。改进:write_slides 完成后工具层**调主 LLM 一次非流式**,输入 deck content 前 2KB + system prompt「生成 3 个 image prompt,视觉风格 CLEARLY DIFFERENT,方向由你看上下文自决」,输出 JSON `{ samples: [{ style, prompt }, × 3] }` → 并发出 3 图 → 用户挑。**配套**:sanity check pairwise Levenshtein 相似度 ≥ 0.8 视为雷同 retry 一次;retry 仍雷同接受降级走"换一批"自救。Plan 改动:抉择 1 完全重写 / 抉择 7 调整("manifest.imageGenStyle 样张阶段不参与,落地阶段才接")/ Task B 重写(加 `mood-board/prompt.ts` 模块 + 主 LLM client 调用 + 解析 + 多样性算法)/ 文件结构表新增两文件 / "不做什么"新增"用户手写 style label 自定义" / roadmap.md Phase 11.8 关键决策同步。**额外烧钱**:每轮多 1 次主 LLM 非流式调用 ~$0.005(用户买单),换"产品风格上限不被工程师锁死",划算                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           | 用户原话:"如果用户没有实际业务在出三张风格,因为你给了风格的提示词等于永远限制死了就从这三种风格里面选一种";核心洞察是**风格是 emergent / 跟业务相关 / 应让用户通过实际业务图反馈来挑,而不是从工程师预设里挑**;Claude 同意并立即同步 plan + roadmap                                                                                               |
