...
import type { CanonicalMessage } from '@big-ppt/shared'
import { __trimMessagesForTesting as trimMessages } from '../src/composables/useAIChat'

const sys: CanonicalMessage = {
  role: 'system',
  content: [{ type: 'text', text: 'sys instructions' }],
}

function asstWithToolUse(ids: string[]): CanonicalMessage {
  return {
    role: 'assistant',
    content: ids.map((id) => ({
...
function toolMsg(id: string): CanonicalMessage {
  return {
    role: 'tool',
    content: [{ type: 'tool_result', toolUseId: id, content: '{"success":true}' }],
  }
}
...
/**
 * 校验：每条 role:'tool' 的消息必须紧跟在含匹配 tool_use block 的 assistant 之后。
 * canonical 协议同 OpenAI/GLM 一样不能让 tool result 孤悬。
 */
function assertNoOrphanTool(messages: CanonicalMessage[]): void {
      ...
      for (const b of m.content) {
        if (b.type === 'tool_result') {
          ...
          if (!pendingIds.has(id)) {
            throw new Error(`orphan tool message toolUseId=${id}`)
          }
...
describe('trimMessages (canonical)', () => {
  it('对短会话直接原样返回', () => {
    const msgs: CanonicalMessage[] = [sys, userMsg('hi'), asstWithToolUse(['a']), toolMsg('a')]
  ...
  it('裁剪长会话时不留下孤儿 tool 消息（复现 GLM "messages 参数非法" bug）', () => {
    ...
    const msgs: CanonicalMessage[] = [
      sys,
      userMsg('帮我做幻灯片'),
      asstWithToolUse(['a1', 'a2']),
      toolMsg('a1'),
      toolMsg('a2'),
    ]
    // 再塞 20 条 (assistant tool_use=1 + tool) 配对，把总长推到触发裁剪的阈值
    for (let i = 0; i < 10; i++) {
      msgs.push(asstWithToolUse([`c${i}`]))
      msgs.push(toolMsg(`c${i}`))
    }
    // 此时 length = 5 + 20 = 25，slice(-20) 会从原 [5] (asstWithToolUse c0) 开始
  ...
  it('保留最末 turn 完整：assistant.tool_use + 其全部 tool 结果不被切散', () => {
    // 末尾构造一组 assistant(tc=3) + 3 个 tool，确保整组保留
    const msgs: CanonicalMessage[] = [sys]
    for (let i = 0; i < 10; i++) {
      msgs.push(asstWithToolUse([`x${i}`]))
      msgs.push(toolMsg(`x${i}`))
    }
    msgs.push(asstWithToolUse(['last1', 'last2', 'last3']))
