...
label : value 配对展示，columns 控制几列布局。

| Prop       | 类型 / 说明                                    |
| ---------- | ---------------------------------------------- |
| `items`    | `Array<{ label: string; value: string }>` 必填 |
| `columns?` | number，默认 `2`                               |
...
三种 `type` 切换语气：info / warning / success。info 走品牌色；warning / success 走中性 amber / green，避免与品牌色冲突。

| Prop / Slot | 类型 / 说明                                       |
| ----------- | ------------------------------------------------- |
| `type?`     | `'info' \| 'warning' \| 'success'`，默认 `'info'` |
| `title?`    | string，标题                                      |
| default     | 正文                                              |

### `<BarChart>` 柱状图

数据可视化基础组件；接收 `labels` + `values` 两个数组。颜色 / 字体读 `--ld-color-chart-primary-*` + `--ld-color-fg-*` + `--ld-font-family-ui` token，无 token 注入时 fallback 中性灰。
...
接口同 `<BarChart>`；折线下方填充色 + 折线/点颜色读 `--ld-color-chart-primary-*` token。
