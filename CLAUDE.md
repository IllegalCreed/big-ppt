...
pnpm gen:image-styles                     # 调整系统配图风格 SVG 后重生成 1280×624 PNG reference
```
...
- **drizzle-kit push 非 TTY 遇交互确认会崩溃但 exit 0，`|| exit` 拦不住**：schema diff 含风险变更（如给有数据的表加唯一索引）时 push 弹确认框（"要不要 truncate?"），部署脚本的非 TTY 环境下直接崩且退出码 0 → 部署继续 pm2 reload → 生产「新代码 + 旧 schema」裂窗，drizzle select 显式列新列名全部 `ER_BAD_FIELD_ERROR` 500。`deploy.sh` 已加两层防御：grep `Interactive prompts require a TTY` 签名 + **`db:verify:prod` reload 前置门**（`packages/agent/scripts/verify-db-schema.mjs`，以 dist schema 为准对比 information_schema 的表/列/索引，缺失即中止且旧进程保持运行）。带风险 schema 变更的正确姿势：**先手工幂等 DDL 再部署**（绝不选 truncate；加唯一索引时新列全 NULL 天然安全，MySQL 唯一索引允许多 NULL），套路见 [docs/runbooks/deploy.md](docs/runbooks/deploy.md)（2026-07-14 Phase 17 部署踩，窗口期 3 个读请求 500）
