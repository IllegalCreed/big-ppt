...
- chart 组件读 CSS 变量（`--bt-*` / `--jyd-*`）按主题取色；远期会下沉到公共组件库读 `--ld-*`（详见 vision.md）
...
详见 [`docs/requirements/roadmap.md`](docs/requirements/roadmap.md)。当前进度：Phase 1–7 ✅，Phase 7.5（模板分层重构 POC）/ 8（依赖升级）/ 9（安全 audit）/ 10（首次部署）排队中。
