...
- **Thinking-tier 模型 maxTokens 计入 thinking budget**：gpt-5.2 / gemini-2.5-flash / Codex-opus-4-7 thinking 模型，`maxTokens` 是「thinking + output」总额。给 50 tokens 跑会触发 MAX_TOKENS finish reason 且 output 空。规则：用 thinking-tier 模型时 `maxTokens` 至少 **2x 预期 output**（[plan 26](docs/plans/26-phase12-multi-llm-providers.md) 踩坑 4）
